...
window.onload = function() {
  ...
  document.getElementById("submitImprovement").addEventListener("click", async function() {
    await new Promise(resolve => {
      document.getElementById("sketchArea").style.display = "none"
      document.getElementById("waitForOthers").style.display = "flex"
      socket.emit("improve", playerID, lobbyCode, canvas.toDataURL(), (redirectBool) => {
        redirect = redirectBool
        if (redirect==true){
          socket.emit("redirectToVote2", lobbyCode)
        }
        resolve(redirect)
      })
  })
...
async function setup() {
  ...
  await new Promise(resolve => {
    socket.emit("requestPNG", lobbyCode, playerID, "Improvement", (images) => {
